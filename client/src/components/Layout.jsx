        ...
        >
          ...
          {rooms.length > 0 && (
            <>
              ...
              {!collapsed && (
                <p className="px-3 pb-1 text-[10px] font-bold uppercase tracking-wider" style={{ color: 'var(--text-secondary)' }}>Open parties</p>
              )}
              <div className="flex flex-col gap-1 overflow-y-auto">
                {rooms.map((r) => {
                  const active = path === `/party/${r.id}`
                  return (
                    <div key={r.id} className="group relative">
                      <Link
                        ...
                        title={`Party ${r.id}`}
                      >
                        <span className="w-6 h-6 rounded-md grid place-items-center text-[9px] font-bold shrink-0"
                          style={{ background: 'rgb(var(--accent-2) / 0.25)', color: 'var(--text-primary)' }}>
                          {r.id.slice(0, 2).toUpperCase()}
                        </span>
                        {!collapsed && <span className="font-mono text-xs truncate">{r.id}</span>}
                      </Link>
                      {!collapsed && (
                        <button
                          onClick={() => remove(r.id)}
                          className="absolute right-2 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity p-1 rounded hover:bg-[rgb(var(--accent-2)/0.2)]"
